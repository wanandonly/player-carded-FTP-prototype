...
export function ResultsScreen({
  ...
  const outcomesByPlayerId = new Map(
    current.outcomes.map((o) => [o.playerId, o]),
  );
  const predictionsByPlayerId = new Map(
    current.predictions.map((p) => [p.playerId, p.willBeBooked]),
  );
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-400">
                    You predicted {prediction ? "booked" : "not booked"} —{" "}
                    {outcomeText}
                  </p>
                </div>
              </div>
